...
test_listener_attr(void)
{
    ...
    ASSERT_OK(seal_get_listener_pos(actual_pos, actual_pos + 1,
                                    actual_pos + 2));
    ASSERT(memcmp(default_pos, actual_pos, sizeof (actual_pos)) == 0);
    ASSERT_OK(seal_set_listener_pos(pos[0], pos[1], pos[2]));
    ASSERT_OK(seal_get_listener_pos(actual_pos, actual_pos + 1, actual_pos + 2));
    ...
    ASSERT_OK(seal_get_listener_orien(actual_orien));
    ASSERT(memcmp(default_orien, actual_orien, sizeof (actual_orien)) == 0);
    ASSERT_OK(seal_set_listener_orien(orien));
    ASSERT_OK(seal_get_listener_orien(actual_orien));
    ASSERT(memcmp(orien, actual_orien, sizeof (actual_orien)) == 0);

    ASSERT_OK(seal_get_listener_gain(&gain));
    ASSERT(gain == 1.0f);
    ASSERT_OK(seal_set_listener_gain(332.0321f));
    ASSERT(seal_set_listener_gain(-0.15f) == SEAL_BAD_VAL);
    ASSERT_OK(seal_get_listener_gain(&gain));
    ASSERT(gain == 332.0321f);
